...
export default function ForecastingPage() {
  ...
  const handleDateRangeChange = (range: { start: number; end: number }) => {
    setDateRange(range);
  };
    ...
    <div>
      ...
      <div>
        ...
        <div>
          ...
          <input type="range" min="1" max={horizon} value={dateRange.start} onChange={(e) => handleDateRangeChange({ start: parseInt(e.target.value), end: dateRange.end })} />
          <input type="range" min="1" max={horizon} value={dateRange.end} onChange={(e) => handleDateRangeChange({ start: dateRange.start, end: parseInt(e.target.value) })} />
        </div>
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={forecastData.slice(dateRange.start - 1, dateRange.end)}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="month" />
            <YAxis />
            <Tooltip />
